...
- Budget: workflows split into two camps. The CLI rejects the wrong
  flag for each camp (mutually exclusive, neither has a default).
  - **Research-call workflows** use `--budget N` (small int, counts
    rumil dispatched calls):
