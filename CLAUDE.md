...
**Declarative CallSpec (Phase 0 only)** (`src/rumil/calls/spec.py`): The scaffolding for a future refactor that collapses the 34 imperative call-type modules into a single `SPECS: dict[(CallType, variant), CallSpec]` registry. A `CallSpec` names the prompt, the three stage IDs (context builder / updater / reviewer), the allowed moves preset, and the dispatch/output contract. Today the legacy registries in `call_registry.py` and `dispatches.py` are still the active source of truth — `SPECS` is empty, nothing reads it, and the module is deliberately additive so it can land without breaking anything. When writing new call types, keep using the existing `CallRunner` subclass pattern; the spec shape is still being validated against real call variants (find_considerations mode, assess default vs big, web_research allowed_domains, ingest source_page) before the imperative path is retired.
