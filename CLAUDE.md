...
**Still on `DB` directly:** the mutation-log machinery itself (`record_mutation_event`, `_load_mutation_state`, `_apply_page_events`, `_apply_link_events`, `_staged_filter`, `StagedOverlay`), `stage_run` / `commit_staged_run`, and the low-level `_execute` / `fork` / `close` plumbing. Hardening these into a `MutationLog` capability object that structurally enforces "never bypass event recording" is the remaining phase of this refactor; today the invariant is still a code-level convention.
