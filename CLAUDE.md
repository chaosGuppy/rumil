...
**Declarative CallSpec (Phases 0-1)** (`src/rumil/calls/spec.py` + `spec_runner.py`): The scaffolding for a future refactor that collapses the 34 imperative call-type modules into a single `SPECS: dict[(CallType, variant), CallSpec]` registry. A `CallSpec` names the prompt, the three stage IDs (context builder / updater / reviewer), the allowed moves, and the dispatch/output contract. `SpecCallRunner` (in `spec_runner.py`) is the generic `CallRunner` subclass that reads its wiring from a spec: it resolves `StageRef`s through the per-stage registries (`CONTEXT_BUILDERS` / `WORKSPACE_UPDATERS` / `CLOSING_REVIEWERS`), plumbs `FromCallParam` / `FromSettings` / `FromStageCtx` sentinels in configs, and resolves `PresetKey` / `PresetOverlay` / explicit tuples to a concrete `MoveType` list. The three stage registries are still empty — per-spec conversions register factories under documented ids as they land. Today the legacy `call_registry.py` / `dispatches.py` are the active source of truth; `SPECS` is empty and nothing imports `SpecCallRunner` yet. When writing new call types, keep using the existing `CallRunner` subclass pattern; the spec shape is still being validated against real call variants (find_considerations mode, assess default vs big, web_research allowed_domains, ingest source_page) before the imperative path is retired.
...
**Prompt versioning**: every call's rendered system prompt is content-hashed (pre-date-suffix) and upserted into the `prompt_versions` table at exchange-save time (see `CallStore.save_llm_exchange`). The hash is stamped on `call_llm_exchanges.composite_prompt_hash`. Filesystem `prompts/` remains the authoring surface; the DB mirror is append-only — edit a .md file and the next call writes a new hash row automatically. The daily `IMPORTANT: Today's date is …` suffix added by `_with_date_suffix` is stripped before hashing so identical content across days produces the same hash. Phase 1 is log-only (no consumer reads the hash yet); Phase 2+ (backfill, A/B by prompt hash, trace-UI exposure, `calls.primary_prompt_hash`) are pending.
...
**Evals as feedback signal** (`src/rumil/evaluate/`, `src/rumil/run_eval/`, `src/rumil/ab_eval/`, `src/rumil/calls/confusion_scan.py`, `src/rumil/orchestrators/policies/eval_feedback.py`): closing the loop from eval outputs back into orchestration. Phase 1 is live: `confusion_scan` heuristics run at every call's completion and write one `reputation_events` row with `source="confusion_scan"`, `dimension="confusion"`, `extra.subject_call_id`/`subject_run_id`. Zero LLM cost. Batched readers live on `AnnotationStore.get_eval_summary_for_pages` / `get_eval_summary_for_calls`, backed by partial expression indexes from `20260419093830_eval_feedback_indexes`. Phase 2 (surfacing quality signals into prioritization context) is partially wired through `two_phase.py` scoring and `EvalDimensionSummaryItem` on `ScoringCompletedEvent`; the matching prompt changes are in `two_phase_main_phase_prioritization.md`, `global_prio.md`, `claim_investigation_p2.md`. The `EvalFeedbackPolicy` (`src/rumil/orchestrators/policies/eval_feedback.py`) is scaffold-only — gated off by `settings.eval_feedback_enabled=False`. Never turn that flag on without first running enough log-only to understand the signal; concentration risk + self-reinforcing eval loops are real. When dispatching from an eval policy, always log the eval event ID provenance into the dispatch `reason` field.

**Data layer** (`src/rumil/database.py` + `src/rumil/db/`): Supabase (Postgres) via the `supabase` Python SDK. Tables: pages, page_links, calls, budget, page_ratings, page_flags, mutation_events, runs, projects, annotation_events, reputation_events, chat_conversations, chat_messages, suggestions, prompt_versions, ab_eval_reports, run_eval_reports, page_format_events, call_costs, run_checkpoints. `DB.create(run_id, prod=True, staged=False)` classmethod handles connection setup (delegated to `settings.py`); defaults to local Supabase. When `staged=True`, writes tag rows with `staged`/`run_id` and mutations go to `mutation_events` (see "Staged Runs and the Mutation Log" above). Several operations use Postgres RPC functions defined in the migrations.
...
- `rumil.db.page_store.PageStore` — page CRUD (`save_page`, `update_page_{importance,content,abstract,task_shape}`, `merge_page_extra`, `supersede_page`, `workspace_coverage`), read helpers (`get_page`, `get_pages_*`, `get_pages_paginated`, `get_pages_slim`), ID/short-ID resolution (`resolve_page_id(s)`, `resolve_link_id`, `page_label`), supersession-chain walking, and project-scoped aggregates (`get_root_questions`, `get_human_questions`, `count_pages_for_question`, `get_assess_staleness`, `count_pages_since`). The `update_page_content` and `supersede_page` paths are mutation-tracked: they call `self._db.record_mutation_event(...)` before dual-writing to base tables (non-staged only).
- `rumil.db.link_store.LinkStore` — link CRUD (`save_link`, `delete_link`, `update_link_role`), traversals (`get_link(s)_to(_many)`, `get_link(s)_from(_many)`, `get_links_between`, `get_all_links`, `_find_duplicate_link`), relationship helpers (considerations, judgements, tension verdicts, dependents/dependencies, child questions, views, inlays, view items, stale deps, dependency counts), and supersession-magnitude lookups on the mutation-events log. `delete_link` and `update_link_role` are mutation-tracked — same record-then-dual-write pattern.

**`rumil.db.mutation_log.MutationLog`** — the staged-runs capability. Owns `staged_filter(query)`, `record(event_type, target_id, payload)` (writes a `mutation_events` row + invalidates cache), `load_state()` (returns the decoded `MutationState` for this run), `apply_page_events(pages)` / `apply_link_events(links)` (overlay the state onto read batches), and `invalidate_cache()`. Installed as `db.mutation_log`; `DB` keeps back-compat shims (`_staged_filter`, `record_mutation_event`, `_load_mutation_state`, `_apply_*_events`, `_invalidate_mutation_cache`) so existing call sites are unchanged. The still-open layer is a `ReadOnlyClient` wrapper that refuses `.update()` / `.delete()` on mutation-tracked tables at pyright-time — that would flip the "never bypass event recording" invariant from a code-review convention into a structural guarantee.

**Still on `DB` directly:** `stage_run` / `commit_staged_run` (they replay the event log across multiple tables and are tightly coupled to `delete_run_data`; a future phase moves them onto `MutationLog`), the `StagedOverlay` helper (wraps read paths; uses `mutation_log` internally), and the low-level `_execute` / `fork` / `close` plumbing (concurrency semaphore, retry, connection lifecycle).

When adding DB methods: put them in the relevant store, add a one-line delegating shim on `DB` to preserve the public surface. Methods that mutate tracked state (`pages`, `page_links`) MUST call `self._db.record_mutation_event(...)` first and only dual-write to the base table when `not self._db.staged` — see `PageStore.supersede_page` / `update_page_content` and `LinkStore.delete_link` / `update_link_role` for the canonical pattern.
...
**Parma** (`parma/`): Separate Next.js operator app on port 3100. Uses a hand-written `src/lib/api.ts` fetch layer (no generated SDK) for domain-specific error ergonomics, but **types** are codegen'd from the same OpenAPI spec into `parma/src/api/` — `parma/src/lib/types.ts` re-exports the generated names. The shared `./scripts/generate-api-types.sh` regenerates both frontend and parma, and the PostToolUse hook on `schemas.py` / `models.py` runs it. Do not edit `parma/src/api/types.gen.ts` by hand. For new types, prefer letting them flow from the OpenAPI spec rather than hand-writing in `parma/src/lib/types.ts` — the hand-written section there is only for response shapes the backend hasn't declared a `response_model` for yet.
