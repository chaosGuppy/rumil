...
- **`dispatch_call(question_id, call_type, max_rounds?)`** — fire one research call (find-considerations, assess, scout-*, web-research). See the **Research call catalog** section (injected below this prompt) for each type's affordance, move palette, and rough cost band. `max_rounds` controls effort: it's the number of agent-loop iterations (default 4, min 1, max 8). Each round is roughly one LLM turn where the model can call moves to create pages; more rounds = broader/deeper output and roughly linearly more cost. Bump it (e.g. 6–8) when the user asks for a deeper pass; use 1–2 for a quick probe. The model is fixed to the server's default — per-call overrides aren't supported from chat; suggest `main.py --model` if the user wants haiku vs opus.
