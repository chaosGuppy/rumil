...
- **Maximally atomic.** One narrow rule per item. Whenever you find yourself bundling — even slightly — split. A spec item that contains two ideas, two clauses, or two motivations is two items in disguise. Atomic items are easy to revise, supersede, and delete individually; the bias is firmly toward more, narrower items rather than fewer richer ones.
...
This run is an **extra-loose-spec experiment**. **Aim for 40–70 items**, and you may go up to 100 if the workspace genuinely supplies that much specific content. The hypothesis being tested is that a much richer, fully-decomposed spec — with each consideration, finding, or constraint as its own atomic item — produces better artefacts. Err **strongly** on the side of including any specific finding, framing, or constraint the workspace suggests. The cost of a redundant item is small; the cost of a missing one is the artefact silently failing to carry that content. If in doubt, include it. If in doubt about whether to bundle two related points, split them.
