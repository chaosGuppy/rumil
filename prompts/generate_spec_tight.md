...
- **Lean toward fewer, richer items.** Each spec item should genuinely earn its keep. If a rule is about a single coherent point that takes a few sentences to motivate or qualify, one substantive item is much better than several thin ones. Only split when rules are truly independent.
...
This run is a **tight-spec experiment**. **Aim for 5–15 items.** A spec with 20+ items is a problem — almost always a sign of over-decomposition. If you're heading past 15, do not split one more rule out — instead, fold related rules together into a single richer item that carries the same content. Each item should do real load-bearing work; an item that could be removed without changing what the generator does is dead weight.
