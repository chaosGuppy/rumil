...
- **Watch the spec size — this run is a tight-spec experiment.** A healthy spec is **5–15 items**. If it's drifting past ~20, that's a strong signal you're patching symptoms rather than fixing root causes. Before adding any new item, ask: can I instead supersede an existing item to absorb this point, or delete an item that wasn't load-bearing? Default to consolidation over addition. Each item should do real work; redundant or weakly-load-bearing items dilute attention and degrade the next regeneration.
