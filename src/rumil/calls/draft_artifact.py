...
The call is a single-pass render — no agent loop, no tools, no refinement.
Refinement (generate -> critique -> refine) is future work (the `Render`
orchestrator described in marketplace-thread/29-generative-workflow.md).
"""
...
class DraftArtifactCall(CallRunner):
    ...
    def _make_workspace_updater(self) -> WorkspaceUpdater:
        return DraftArtifactUpdater(shape=self._shape)
