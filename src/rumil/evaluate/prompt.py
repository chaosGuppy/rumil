...
def build_evaluation_prompt(eval_type: str = DEFAULT_EVAL_TYPE) -> str:
    ...
    if filename is None:
        raise ValueError(f"Unknown eval type {eval_type!r}. Valid types: {', '.join(EVAL_PROMPTS)}")
...
def build_investigator_prompt(eval_type: str = DEFAULT_EVAL_TYPE) -> str:
    ...
    preamble = (_PROMPTS_DIR / "preamble.md").read_text()
    task_filename = INVESTIGATOR_TASK_PROMPTS.get(eval_type, INVESTIGATOR_TASK_PROMPTS["default"])
