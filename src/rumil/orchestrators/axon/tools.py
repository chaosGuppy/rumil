...
DELEGATE_DESCRIPTION = (
    "Delegate a focused sub-task to a configured inner agent. After you "
    "call this, you'll be asked to produce a full configuration for it "
    "by calling `configure` in the next turn — a follow-up where you'll "
    "see this same conversation plus a directive identifying which "
    "delegate to configure. The inner agent runs with that config, "
    "terminates by calling `finalize`, and its result lands here as "
    "this call's tool_result.\n"
    "\n"
    "Two regimes:\n"
    " - inherit_context=True: the inner agent inherits this conversation "
    "as its prefix and uses the same tools and system prompt I'm using. "
    "Cache-shared continuation. Use when the inner agent is a forked "
    "branch of your own thinking (distill recent discussion, steelman a "
    "current stance, continue exploring a thread).\n"
    " - inherit_context=False: the inner agent starts fresh; configure "
