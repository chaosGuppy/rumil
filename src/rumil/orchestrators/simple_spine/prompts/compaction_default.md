...
2. **Durable findings from spawns.** The actual claims, sources, contradictions, and evidence the subroutines have surfaced. Quote verbatim where precision matters (specific numbers, names, phrasings, source URLs). Drop the spawn mechanics — which subroutine ran, retry chatter, parameter overrides — unless they bear on what to do next.
