"""ExperimentalQuestionPrioritiser: actor for the experimental variant.

Subclass of ``QuestionPrioritiser`` that swaps in the experimental
round-planning behaviour:

* ``summarise_before_assess = False`` (class attribute so
  ``GlobalPrioOrchestrator``'s class-attribute lookup resolves).
* Subquestion-linker subsystem — runs at the start of initial and main
  prio (gated on ``settings.subquestion_linker_enabled`` and the page-count
  cache threshold).
* ``MIN_EXPERIMENTAL_INITIAL_PRIO_BUDGET`` gating — below the threshold,
  initial prio is skipped and the pre-created call is cancelled with a
  specific reason.
* Contextvar-plumbed scout budget — ``set_experimental_scout_budget`` is
  refreshed at the start of every round so nested LLM calls can observe
  the right per-round value. Contextvars are task-local, so sibling
  prioritisers under ``asyncio.gather`` stay isolated.
* Experimental scoring — uses ``ExperimentalSubquestionScore`` +
  ``ExperimentalScoringCompletedEvent`` + ``score_subquestions_experimental``
  prompt. No claim scoring in main phase.
* Main-phase prompt: ``two_phase_main_phase_prioritization_experimental``.
* Round boundary: no per-round view; uses ``assess_question(summarise=False)``
  only when ``_invocation > 1`` (initial phase is invocation 1).
* Recurse: only question recurse (no claim recurse).
"""
...
from rumil.constants import (
    LAST_CALL_THRESHOLD,
    MIN_EXPERIMENTAL_INITIAL_PRIO_BUDGET,
    MIN_TWOPHASE_BUDGET,
)
...
class ExperimentalQuestionPrioritiser(QuestionPrioritiser):
    ...
    async def _run_round(self, round_budget: int) -> None:
        """One experimental prioritisation round.

        Shape mirrors ``QuestionPrioritiser._run_round`` but:
        * refreshes the ``experimental_scout_budget`` contextvar per
          round so nested scout dispatches see the current effective
          budget;
        * no per-round view boundary — uses ``assess_question`` with
          ``summarise=False`` on rounds past the first (``_invocation > 1``
          or ``last_call``).
        """
    ...
    async def _get_next_batch(
        ...
        if self._invocation == 0:
            ...
            if effective_remaining < MIN_EXPERIMENTAL_INITIAL_PRIO_BUDGET:
                await self._cancel_initial_call_with_reason(
            ...
            elif await self._needs_initial_prioritization():
                ...
                await self._cancel_initial_call_with_reason(
                    "Question already has a judgement or view."
                )
            self._executed_since_last_plan = True
    ...
    async def _cancel_initial_call_with_reason(self, reason: str) -> None:
        """Experimental-variant skip: records the reason on the trace."""
    ...
    async def _run_subquestion_linker(self, parent_call_id: str | None) -> None:
        """Run the LinkSubquestionsCall and update the linker eval timestamp.

        Any failure is logged and swallowed — the round must proceed regardless.
        """
        assert self.db is not None
        try:
    ...
    async def _maybe_rerun_linker(self, parent_call_id: str | None) -> None:
        ...
        pages have been added since this prioritiser's last evaluation to
        invalidate the cache.
        """
        assert self.db is not None
        counts = await self.db.get_call_counts_by_type(self.question_id)
        if counts.get(CallType.LINK_SUBQUESTIONS.value, 0) == 0:
            log.info(
                "No prior linker on question=%s, running now",
                self.question_id[:8],
            )
            await self._run_subquestion_linker(parent_call_id)
            return
    ...
    async def _initial_prioritization(
        ...
        log.info(
            "ExperimentalQuestionPrioritiser initial_prioritization: question=%s, budget=%d",
            self.question_id[:8],
        ...
        if get_settings().subquestion_linker_enabled:
            await self._run_subquestion_linker(p_call.id)
        ...
        log.info(
            "ExperimentalQuestionPrioritiser initial_prioritization complete: %d sequences",
            len(sequences),
        )
    ...
    async def _main_phase_prioritization(
        ...
        assert self.db is not None
        log.info(
            "ExperimentalQuestionPrioritiser main_phase_prioritization: "
            "question=%s, budget=%d, last_call=%s",
            self.question_id[:8],
            budget,
            last_call,
        )
        ...
        if get_settings().subquestion_linker_enabled:
            await self._maybe_rerun_linker(p_call.id)
        ...
        log.info(
            "ExperimentalQuestionPrioritiser main_phase_prioritization complete: "
            "%d sequences, %d recurses",
            len(sequences),
            len(recurses),
        )
        return PrioritizationResult(
