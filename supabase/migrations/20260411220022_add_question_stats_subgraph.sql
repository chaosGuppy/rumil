-- Extend compute_question_stats to also emit the 3-hop subgraph structure
-- (nodes + edges) so the frontend can visualize the neighborhood. Baseline-
-- only, same staging assumptions as the original RPC.
--
-- Nodes: one row per active page in the hood, with its minimum hop distance
-- from the anchor question. Edges: both endpoints projected for the
-- frontend, keyed by link_type.
