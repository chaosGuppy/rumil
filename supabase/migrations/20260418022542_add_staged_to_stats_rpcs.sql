-- B04: add p_staged_run_id parameter to the stats RPCs so staged runs see
-- baseline + own-written pages/links, mirroring the match_pages /
-- get_root_questions pattern from migration 20260328102907.
--
-- Visibility predicate everywhere we read pages, page_links, or calls:
--     (staged = FALSE OR run_id = p_staged_run_id)
--
-- Pass NULL (the default) for baseline-only queries.
