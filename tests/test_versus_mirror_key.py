...
# Submit-judgment path writes order on the row ------------------------------


def test_submit_judgment_writes_order_on_row(tmp_path, monkeypatch):
    """End-to-end guard for the human-judge submit endpoint's row builder.

    Goes through submit_judgment by calling the endpoint function
    directly with a tmp judgments log, then reads the row back and
    checks that ``order`` was written.
    """
    import datetime as dt

    from rumil.api import versus_router

    # Stub cfg to point judgments_log at a fresh tmp file.
    judgments_log = tmp_path / "judgments.jsonl"

    @dataclass
    class _Storage:
        judgments_log: Path
        completions_log: Path
    ...
    fake_cfg = _Cfg(
        storage=_Storage(
            judgments_log=judgments_log, completions_log=tmp_path / "completions.jsonl"
        )
    )

    monkeypatch.setattr(versus_router, "_cfg_required", lambda: fake_cfg)
    monkeypatch.setattr(versus_router, "_resolve_path", lambda p: p)
    # Bypass datetime call for stability (not strictly needed).
    monkeypatch.setattr(dt, "datetime", dt.datetime)

    body = versus_router.JudgmentSubmit(
        name="brian",
        essay_id="e1",
        prefix_hash="ph1",
        a="human",
        b="openai/gpt-5.4",
        first_source="openai/gpt-5.4",  # higher-sorted first -> order "ba"
        second_source="human",
        criterion="general_quality",
        verdict="A",
        note="",
    )
    result = versus_router.submit_judgment(body)

    from versus import jsonl as versus_jsonl

    rows = list(versus_jsonl.read(judgments_log))
    assert len(rows) == 1
    assert rows[0]["order"] == "ba"
    assert rows[0]["key"] == result.key
    assert result.key.endswith("|ba")
